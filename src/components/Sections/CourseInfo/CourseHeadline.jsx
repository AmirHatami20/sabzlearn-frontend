...
import {useGetSessions} from "../../../hooks/react-query/session.js";
...
function CourseHeadline({courseId}) {
    ...
    const {data: courseSections = [], isLoading} = useGetSessions(courseId);
    ...
    return (courseSections.map((section) => {
            ...
            return (<div key={section._id}>
                    {/* Section Header */}
                    <div
                        className={`flex justify-between items-center p-4 rounded-t-lg cursor-pointer transition-all ${showContentId === section._id ? "bg-gray-500 dark:bg-primary text-white" : "bg-gray-100 dark:bg-secendery-dark"}`}
                    ...
                    >
                        ...
                        <div className="flex items-center gap-x-2.5 shrink-0">
                            <div
                                className={`hidden lg:flex items-center gap-x-1.5 text-sm transition-all duration-20 ${showContentId === section._id ? "text-white" : "text-slate-500 dark:text-white"}`}>
                            ...
                            <LiaAngleDownSolid
                                className={`text-xl transition-transform duration-200 ${showContentId === section._id ? "rotate-180" : ""}`}
                            />
                        </div>
                    </div>

                    {/* Section Sessions */}
                    {showContentId === section._id && section.sessions.length > 0 && (<div>
                            {section.sessions.map((session, index) => (<div
                                    key={session._id}
